...
export class Migration20220330135528 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table `phala_account` (`id` int unsigned not null auto_increment primary key, `address` varchar(255) not null, `identity` varchar(255) null, `identity_verified` tinyint(1) not null, `created_at` datetime not null, `updated_at` datetime not null, `discr` enum(\'account\', \'stats_account\', \'watchdog_account\') not null) default character set utf8mb4 engine = InnoDB;');
    this.addSql('alter table `phala_account` add unique `phala_account_address_unique`(`address`);');
    this.addSql('alter table `phala_account` add index `phala_account_discr_index`(`discr`);');

    this.addSql('create table `phala_stakepool` (`id` int unsigned not null auto_increment primary key, `on_chain_id` int(11) null, `owner_id` int(11) unsigned not null, `created_at` datetime not null, `updated_at` datetime not null, `discr` enum(\'stake_pool\', \'stats_stake_pool\', \'watchdog_stake_pool\') not null, `special` varchar(255) null, `last_history_entry_id` int(11) unsigned null) default character set utf8mb4 engine = InnoDB;');
    ...
    this.addSql('alter table `phala_stakepool` add index `phala_stakepool_discr_index`(`discr`);');
    this.addSql('alter table `phala_stakepool` add index `phala_stakepool_last_history_entry_id_index`(`last_history_entry_id`);');
    this.addSql('alter table `phala_stakepool` add unique `phala_stakepool_last_history_entry_id_unique`(`last_history_entry_id`);');
    ...
    this.addSql('create table `phala_stakepool_issues` (`stake_pool_id` int(11) unsigned not null, `issue_id` int(11) unsigned not null) default character set utf8mb4 engine = InnoDB;');
    this.addSql('alter table `phala_stakepool_issues` add index `phala_stakepool_issues_stake_pool_id_index`(`stake_pool_id`);');
    this.addSql('alter table `phala_stakepool_issues` add index `phala_stakepool_issues_issue_id_index`(`issue_id`);');
    this.addSql('alter table `phala_stakepool_issues` add primary key `phala_stakepool_issues_pkey`(`stake_pool_id`, `issue_id`);');
    ...
    this.addSql('alter table `phala_stakepool` add constraint `phala_stakepool_last_history_entry_id_foreign` foreign key (`last_history_entry_id`) references `stats_stakepool_historyentry` (`id`) on update cascade on delete set null;');

    this.addSql('alter table `stats_stakepool_historyentry` add constraint `stats_stakepool_historyentry_stake_pool_id_foreign` foreign key (`stake_pool_id`) references `phala_stakepool` (`id`) on update cascade;');
    ...
    this.addSql('alter table `phala_stakepool_issues` add constraint `phala_stakepool_issues_stake_pool_id_foreign` foreign key (`stake_pool_id`) references `phala_stakepool` (`id`) on update cascade on delete cascade;');
    this.addSql('alter table `phala_stakepool_issues` add constraint `phala_stakepool_issues_issue_id_foreign` foreign key (`issue_id`) references `stats_stakepool_issue` (`id`) on update cascade on delete cascade;');

    this.addSql('alter table `stats_event` add constraint `stats_event_stake_pool_id_foreign` foreign key (`stake_pool_id`) references `phala_stakepool` (`id`) on update cascade on delete set null;');
    ...
    this.addSql('alter table `stats_worker` add constraint `stats_worker_stake_pool_id_foreign` foreign key (`stake_pool_id`) references `phala_stakepool` (`id`) on update cascade on delete set null;');
    ...
    this.addSql('alter table `watchdog_user_accounts` add constraint `watchdog_user_accounts_account_id_foreign` foreign key (`account_id`) references `phala_account` (`id`) on update cascade on delete cascade;');
