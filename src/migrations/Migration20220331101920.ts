...
export class Migration20220331101920 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table `watchdog_account` (`id` int unsigned not null auto_increment primary key, `address` varchar(255) not null, `identity` varchar(255) null, `identity_verified` tinyint(1) not null, `created_at` datetime not null, `updated_at` datetime not null) default character set utf8mb4 engine = InnoDB;');
    this.addSql('alter table `watchdog_account` add unique `watchdog_account_address_unique`(`address`);');

    this.addSql('create table `watchdog_user` (`id` int unsigned not null auto_increment primary key, `msg_channel` enum(\'discord\', \'telegram\') not null, `msg_user_id` varchar(255) not null, `token` varchar(255) null, `username` varchar(255) not null, `created_at` datetime not null, `updated_at` datetime not null, `config` json not null) default character set utf8mb4 engine = InnoDB;');
    ...
    this.addSql('create table `watchdog_user_accounts` (`user_id` int(11) unsigned not null, `watchdog_account_id` int(11) unsigned not null) default character set utf8mb4 engine = InnoDB;');
    this.addSql('alter table `watchdog_user_accounts` add index `watchdog_user_accounts_user_id_index`(`user_id`);');
    this.addSql('alter table `watchdog_user_accounts` add index `watchdog_user_accounts_watchdog_account_id_index`(`watchdog_account_id`);');
    this.addSql('alter table `watchdog_user_accounts` add primary key `watchdog_user_accounts_pkey`(`user_id`, `watchdog_account_id`);');
    ...
    this.addSql('create table `stats_stakepool` (`id` int unsigned not null auto_increment primary key, `stake_pool_id` int(11) unsigned not null, `special` varchar(255) null, `on_chain_id` int(11) null, `owner_id` int(11) unsigned not null, `last_history_entry_id` int(11) unsigned null, `created_at` datetime not null, `updated_at` datetime not null) default character set utf8mb4 engine = InnoDB;');
    this.addSql('alter table `stats_stakepool` add index `stats_stakepool_stake_pool_id_index`(`stake_pool_id`);');
    this.addSql('alter table `stats_stakepool` add unique `stats_stakepool_stake_pool_id_unique`(`stake_pool_id`);');
    this.addSql('alter table `stats_stakepool` add index `stats_stakepool_owner_id_index`(`owner_id`);');
    this.addSql('alter table `stats_stakepool` add index `stats_stakepool_last_history_entry_id_index`(`last_history_entry_id`);');
    this.addSql('alter table `stats_stakepool` add unique `stats_stakepool_last_history_entry_id_unique`(`last_history_entry_id`);');
    ...
    this.addSql('create table `stats_stakepool_issues` (`stats_stake_pool_id` int(11) unsigned not null, `issue_id` int(11) unsigned not null) default character set utf8mb4 engine = InnoDB;');
    this.addSql('alter table `stats_stakepool_issues` add index `stats_stakepool_issues_stats_stake_pool_id_index`(`stats_stake_pool_id`);');
    this.addSql('alter table `stats_stakepool_issues` add index `stats_stakepool_issues_issue_id_index`(`issue_id`);');
    this.addSql('alter table `stats_stakepool_issues` add primary key `stats_stakepool_issues_pkey`(`stats_stake_pool_id`, `issue_id`);');
    ...
    this.addSql('create table `watchdog_stake_pool` (`id` int unsigned not null auto_increment primary key, `on_chain_id` int(11) null, `owner_id` int(11) unsigned not null, `created_at` datetime not null, `updated_at` datetime not null) default character set utf8mb4 engine = InnoDB;');
    this.addSql('alter table `watchdog_stake_pool` add unique `watchdog_stake_pool_on_chain_id_unique`(`on_chain_id`);');
    this.addSql('alter table `watchdog_stake_pool` add index `watchdog_stake_pool_owner_id_index`(`owner_id`);');
    ...
    this.addSql('create table `watchdog_stakepool_observation` (`id` int unsigned not null auto_increment primary key, `user_id` int(11) unsigned not null, `stake_pool_id` int(11) unsigned not null, `account_id` int(11) unsigned null, `mode` enum(\'owner\', \'delegator\') null, `config` json not null, `last_notifications` json not null) default character set utf8mb4 engine = InnoDB;');
    ...
    this.addSql('alter table `watchdog_user_accounts` add constraint `watchdog_user_accounts_watchdog_account_id_foreign` foreign key (`watchdog_account_id`) references `watchdog_account` (`id`) on update cascade on delete cascade;');
    ...
    this.addSql('alter table `stats_stakepool` add constraint `stats_stakepool_stake_pool_id_foreign` foreign key (`stake_pool_id`) references `phala_stakepool` (`id`) on update cascade;');
    this.addSql('alter table `stats_stakepool` add constraint `stats_stakepool_owner_id_foreign` foreign key (`owner_id`) references `phala_account` (`id`) on update cascade;');
    this.addSql('alter table `stats_stakepool` add constraint `stats_stakepool_last_history_entry_id_foreign` foreign key (`last_history_entry_id`) references `stats_stakepool_historyentry` (`id`) on update cascade on delete set null;');

    this.addSql('alter table `stats_event` add constraint `stats_event_stake_pool_id_foreign` foreign key (`stake_pool_id`) references `stats_stakepool` (`id`) on update cascade on delete set null;');
    ...
    this.addSql('alter table `stats_stakepool_historyentry` add constraint `stats_stakepool_historyentry_stake_pool_id_foreign` foreign key (`stake_pool_id`) references `stats_stakepool` (`id`) on update cascade;');

    this.addSql('alter table `stats_stakepool_issues` add constraint `stats_stakepool_issues_stats_stake_pool_id_foreign` foreign key (`stats_stake_pool_id`) references `stats_stakepool` (`id`) on update cascade on delete cascade;');
    this.addSql('alter table `stats_stakepool_issues` add constraint `stats_stakepool_issues_issue_id_foreign` foreign key (`issue_id`) references `stats_stakepool_issue` (`id`) on update cascade on delete cascade;');
    ...
    this.addSql('alter table `stats_worker` add constraint `stats_worker_stake_pool_id_foreign` foreign key (`stake_pool_id`) references `stats_stakepool` (`id`) on update cascade on delete set null;');

    this.addSql('alter table `watchdog_stake_pool` add constraint `watchdog_stake_pool_owner_id_foreign` foreign key (`owner_id`) references `phala_account` (`id`) on update cascade;');

    this.addSql('alter table `watchdog_issue_uworker` add constraint `watchdog_issue_uworker_stake_pool_id_foreign` foreign key (`stake_pool_id`) references `watchdog_stake_pool` (`id`) on update cascade;');
    ...
    this.addSql('alter table `watchdog_stakepool_observation` add constraint `watchdog_stakepool_observation_stake_pool_id_foreign` foreign key (`stake_pool_id`) references `watchdog_stake_pool` (`id`) on update cascade;');
    this.addSql('alter table `watchdog_stakepool_observation` add constraint `watchdog_stakepool_observation_account_id_foreign` foreign key (`account_id`) references `watchdog_account` (`id`) on update cascade on delete set null;');
